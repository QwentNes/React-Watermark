...
import {useParametersInput} from "../../hook/useParametersInput";
...
const Parameters: React.FC<ParametersProps> = observer(({}) => {
    const {isShow, deleteBtn, SizeInput, SelectInput, SliderInput} = useParametersInput()
...
const EffectItem: React.FC<EffectProps> = ({children, isBottom}) => {
    return (
        <div className={classNames(style.effect, isBottom && style.bottom_effect)}>
            {children}
        </div>
    )
}

const EffectTitle: React.FC = ({children}) => {
    return (
        <div className={style.title}>
            {children}
        </div>
